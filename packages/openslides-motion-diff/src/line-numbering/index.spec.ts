...
describe(`inserting line numbering`, () => {
    it(`does not count empty lines, case 1`, () => {
    ...
    it(`does not count empty lines, case 2`, () => {
        const inHtml = `<ul>\n\n<li>Point 1</li>\n\n</ul>`;
        const outHtml = insert({ html: inHtml, lineLength: 80, firstLine: 1 });
    ...
    describe(`block nodes`, () => {
        it(`leaves a simple DIV untouched`, () => {
            const inHtml = `<div>Test</div>`;
            const outHtml = insert({
                html: inHtml,
                lineLength: 5,
                firstLine: 1
            });
            expect(outHtml).toBe(`<div>` + noMarkup(1) + `Test</div>`);
            expect(strip(outHtml)).toBe(inHtml);
            expect(insertLineBreaks(outHtml, 80)).toBe(outHtml);
        });

        it(`breaks a DIV containing only inline elements`, () => {
        ...
        it(`handles a DIV within a DIV correctly`, () => {
        ...
        it(`ignores white spaces between block element tags`, () => {
            const inHtml = `<ul>\n<li>Test</li>\n</ul>`;
            const outHtml = insert({
                html: inHtml,
                lineLength: 80,
    ...
    describe(`indentation for block elements`, () => {
        ...
        it(`indents BLOCKQUOTE-elements`, () => {
            const inHtml =
        ...
        it(`shortens the line for H1-elements by 2/3`, () => {
        ...
        it(`shortens the line for H2-elements by 0.75`, () => {
        ...
        it(`shortens the line for H3-elements by 0.85`, () => {
        ...
        it(`indents Ps with 30px-padding-left by 6 characters`, () => {
        ...
        it(`indents Ps with 30px-padding-right by 6 characters`, () => {
        ...
        it(`breaks before an inline element, if the first word of the new inline element is longer than the remaining line (1)`, () => {
            const inHtml = `<p>Duis autem vel eum iriure dolor in hendrerit in vulputate velit esse molestie <strong>consequat, vel illum dolore eu feugiat nulla facilisis at vero eros et accumsan et iusto odio</strong>.</p>`;
            const outHtml = insert({ html: inHtml, lineLength: 80, firstLine: 1 });
            expect(outHtml).toBe(
                `<p>` +
                noMarkup(1) +
        ...
        it(`breaks before an inline element, if the first word of the new inline element is longer than the remaining line (2)`, () => {
            const inHtml = `<p><span>Duis autem vel eum iriure dolor in hendrerit in vulputate velit esse molestie <strong>consequat, vel illum dolore eu feugiat nulla facilisis at vero eros et accumsan et iusto odio</strong>.</span></p>`;
            const outHtml = insert({ html: inHtml, lineLength: 80, firstLine: 1 });
            expect(outHtml).toBe(
                `<p>` +
                noMarkup(1) +
        ...
        it(`does not fail in a weird case`, () => {
    ...
    describe(`line numbering: inline nodes`, () => {
        it(`leaves a simple SPAN untouched`, () => {
        ...
        it(`breaks lines in a simple SPAN`, () => {
        ...
        it(`breaks lines in nested inline elements`, () => {
        ...
        it(`counts after DEL/INS-nodes`, () => {
            const inHtml = `<P>leo Testelefantgeweih Buchstabenwut als Achzehnzahlunginer. Hierbei <DEL>darf</DEL><INS>setzen</INS> bist der Deifi <DEL>das </DEL><INS>Dor Reh Wachtel da </INS>Subjunktivier <DEL>als Derftige Aal</DEL><INS>san</INS> Orthopädische<DEL>, der Arbeitsnachweisdiskus Bass der Tastatur </DEL><DEL>Weiter schreiben wie Tasse Wasser als</DEL><INS> dienen</INS>.</P>`;
            const outHtml = insert({
                html: inHtml,
                lineLength: 95,
                firstLine: 1
        ...
        it(`handles STRIKE-tags`, () => {
            const inHtml = `<p>et accusam et justo duo dolores et ea <span style="color: #ff0000;"><strike>rebum </strike></span><span style="color: #006400;">Inserted Text</span>. Stet clita kasd gubergren,</p>`;
            const outHtml = insert({
                html: inHtml,
                lineLength: 80,
                firstLine: 1
        ...
        it(`treats ascii newline characters like spaces`, () => {
    ...
    describe(`line numbering in regard to the inline diff`, () => {
        it(`does not count within INS nodes`, () => {
            const inHtml = `1234 <ins>1234</ins> 1234 1234`;
            const outHtml = insert({ html: inHtml, lineLength: 10, firstLine: 1 });
            expect(outHtml).toBe(noMarkup(1) + `1234 <ins>1234</ins> 1234 ` + brMarkup(2) + `1234`);
            expect(strip(outHtml)).toBe(inHtml);
            expect(insertLineBreaks(outHtml, 80)).toBe(outHtml);
        });

        it(`does not count within .insert nodes`, () => {
        ...
        it(`does not create a new line for a trailing INS`, () => {
            const inHtml = `<p>et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet, consetetur<ins>dsfsdf23</ins></p>`;
            const outHtml = insert({ html: inHtml, lineLength: 80, firstLine: 1 });
            expect(outHtml).toBe(
                `<p>` +
                noMarkup(1) +
        ...
        it(`inserts the line number before the INS, if INS is the first element of the paragraph`, () => {
            const inHtml = `<p><ins>lauthals </ins>'liebe Kinder, ich will hinaus in den Wald, seid auf der Hut vor dem Wolf!' Und noch etwas mehr Text bis zur nächsten Zeile</p>`;
            const outHtml = insert({ html: inHtml, lineLength: 80, firstLine: 1 });
            expect(outHtml).toBe(
                `<p>` +
                noMarkup(1) +
...
describe(`insert line breaks`, () => {
    const plainBr = `<br class="os-line-break">`;

    it(`breaks a DIV containing only inline elements`, () => {
        const inHtml = `<div>Test <span>Test1234</span>5678 Test</div>`;
        const outHtml = insertLineBreaks(inHtml, 5);
        expect(outHtml).toBe(
            `<div>Test ` +
            plainBr +
            `<span>Test1` +
    ...
    it(`indents BLOCKQUOTE-elements`, () => {
        ...
        const expected =
            ...
            plainBr +
            `CDEFGHIJKLMNOPQRSTUV</div>`;
        const outHtml = insertLineBreaks(inHtml, 80);
        expect(outHtml).toBe(expected);
        expect(insertLineBreaks(outHtml, 80)).toBe(outHtml);
    });

    it(`DOES count within INS nodes`, () => {
        const inHtml = `1234 <ins>1234</ins> 1234 1234`;
        const outHtml = insertLineBreaks(inHtml, 10, true);
    ...
    it(`does not create a new line for a trailing INS`, () => {
        const inHtml = `<p>et justo duo dolores et ea rebum. Stet clita kasd gubergren, no sea takimata sanctus est Lorem ipsum dolor sit amet. Lorem ipsum dolor sit amet, consetetur<ins>dsfsdf23</ins></p>`;
        const outHtml = insertLineBreaks(inHtml, 80, true);
        expect(outHtml).toBe(
    ...
    it(`ignores witespaces by previously added line numbers`, () => {
...
describe(`line highlighting`, () => {
    it(`highlights a simple line`, () => {
        const inHtml = insert({
            html: `<span>Lorem ipsum dolorsit amet</span>`,
            lineLength: 5,
            firstLine: 1
        });
        const highlighted = highlightLine(inHtml, 2);
    ...
    it(`highlights a simple line with formattings`, () => {
        const highlighted = insert({
    ...
    it(`highlights the last line`, () => {
        const inHtml = insert({
            html: `<span>Lorem ipsum dolorsit amet</span>`,
            lineLength: 5,
            firstLine: 1
        });
        const highlighted = highlightLine(inHtml, 5);
        expect(highlighted).toBe(
            noMarkup(1) +
            `<span>Lorem ` +
            brMarkup(2) +
    ...
    it(`highlights the first line`, () => {
        ...
        const highlighted = highlightLine(inHtml, 1);
        expect(highlighted).toBe(
            noMarkup(1) +
    ...
    it(`does not change the string if the line number is not found`, () => {
        const inHtml = insert({
            html: `<span>Lorem ipsum dolorsit amet</span>`,
            lineLength: 5,
            firstLine: 1
        });
        const highlighted = highlightLine(inHtml, 8);
        expect(highlighted).toBe(
            noMarkup(1) +
            `<span>Lorem ` +
            brMarkup(2) +
...
/*
    describe(`line numbering: test nodes`, () => {
        ...
        it(`breaks simple lines with offset`, () => {
            const textNode = document.createTextNode(`012345678901234567`);
        ...
        it(`breaks simple lines with offset equaling to length`, () => {
            const textNode = document.createTextNode(`012345678901234567`);
        ...
        it(`breaks simple lines with spaces (1)`, () => {
        ...
        it(`breaks simple lines with spaces (2)`, () => {
        ...
        it(`breaks simple lines with spaces (3)`, () => {
        ...
        it(`breaks simple lines with spaces (4)`, () => {
