...
export class ChessChallengeService {
    constructor(private notifyService: NotifyService, private op: OperatorService, private dialog: MatDialog, private translate: TranslateService, private prompt: PromptService) {}

    public startListening(): void {
        this.notifyService.getMessageObservable(`chess_challenge`).subscribe(async (notify: NotifyResponse<{ name: string }>) => {
            if (!notify.sendByThisUser) {
                const title = notify.message.name + ` ` + this.translate.instant(`challenged you to a chess match!`);
