...
import { FlatNode, TreeIdNode } from '@app/infrastructure/definitions/tree';
...
export class SortingTreeComponent<T extends Identifiable & Displayable> implements OnDestroy {
     ...
     */
    public nextNode: ExFlatNode<T> | null = null;

    /**
     * Pointer for the move event
     */
    private pointer: DragEvent | null = null;
     ...
     */
    private activeFilter: (node: T) => boolean = () => false;
     ...
     */
    public getParentNode(node: FlatNode<T>): FlatNode<T> | null {
    ...
    private getExpandedParentNode(node: FlatNode<T>): FlatNode<T> {
        for (let i = node.position || 0; i >= 0; --i) {
    ...
    private getNextVisibleNode(
        node: FlatNode<T>,
        direction: Direction.DOWNWARDS | Direction.UPWARDS
    ): FlatNode<T> | null {
        if (node) {
            switch (direction) {
                case Direction.DOWNWARDS:
                    for (let i = node.position || 0; i < this.osTreeData.length; ++i) {
    ...
    private getTheLastInvisibleNode(node: FlatNode<T>): FlatNode<T> {
        let result = node;
        for (
            let i = node.position! + 1;
            i < this.osTreeData.length && this.osTreeData[i].level >= node.level + 1;
            ++i
        ) {
            if (this.osTreeData[i].filtered) {
    ...
    private moveItemToTree(node: FlatNode<T>, previousIndex: number, nextIndex: number, nextLevel: number): void {
        ...
        const lastChildIndex = movedNodes[movedNodes.length - 1].position as number;
    ...
    private checkChildrenToBeFiltered(node: FlatNode<T>, parent?: FlatNode<T>): boolean {
        let result = false;
        const willFiltered = this.activeFilter ? this.activeFilter(node.item) || !!parent?.filtered : false;
