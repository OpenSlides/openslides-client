...
export const getMeetingListSubscriptionConfig = (getNextMeetingIdObservable: () => Observable<Id | null>) => ({
    modelRequest: {
        ...
        ids: [ORGANIZATION_ID],
        follow: [
            { idField: `active_meeting_ids`, follow: [{ idField: `group_ids`, isFullList: false }], fieldset: `list` },
            {
                idField: `archived_meeting_ids`,
                follow: [{ idField: `group_ids`, isFullList: false }],
                fieldset: `list`
            },
            { idField: `template_meeting_ids`, follow: [{ idField: `group_ids`, isFullList: false }], fieldset: `list` }
        ]
    },
