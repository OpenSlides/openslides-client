...
export class MotionForwardDialogService extends BaseDialogService<MotionForwardDialogComponent, ViewMotion[], Ids> {
    ...
    public async forwardMotionsToMeetings(...motions: ViewMotion[]): Promise<void> {
        ...
        if (toMeetingIds) {
            try {
                await this.modelRequest.subscribeTo(
                    getMotionForwardDataSubscriptionConfig(...toForward.map(motion => motion.id))
                );
                await this.modelRequest.subscriptionGotData(MOTION_FORWARD_DATA_SUBSCRIPTION);
