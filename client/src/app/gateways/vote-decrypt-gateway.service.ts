        ...
        this._verificationSubscription = this.repo
            ...
            .pipe(
                ...
                distinctUntilChanged((prev, curr) => {
                    this._lastVoteVerificationData = prev.map(date =>
                        (({ options: _, ...others }) => others as VoteVerificationData)(date)
                    );
            ...
            .subscribe(verificationData =>
                this.updateVerification(
                    verificationData.map(date => (({ options: _, ...others }) => others as VoteVerificationData)(date))
                )
            );
    }
